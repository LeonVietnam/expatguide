...
除非是在引用来源，或该词对准确性严格必要且无法自然替换，否则默认不用以下词或短语：
...
输入：TOPIC SHEET + RESEARCH PACK + VALIDATION PACK
输出：DRAFT PACKAGE
...
- absolute_internal_links_only：列出所有内链
...
4. **多一个 Claude 二审步骤**
   ...
   3. Prompt 3 → VALIDATION PACK
   4. Deep Draft Prompt → DRAFT PACKAGE
   5. Claude QA Prompt → ISSUE LIST
   6. Prompt 5 → FINAL PACKAGE

5. **Step 5 的 patch 会更依赖 Claude 问题单**
   深度文 patch 时，除了脚本报错，还要吃 Claude 的 editorial QA 结果。

### 深度车道最小改动原则

如果你想把手册维持得尽量简单，可以这样理解：

- Prompt 1：沿用普通版
- Prompt 2：沿用普通版，但执行时更严格
- Prompt 3：沿用普通版，但执行时更严格
- Prompt 4：单独使用深度文版本
- Claude QA：深度文专用
- Prompt 5：沿用普通版，但 ISSUE LIST 加入 Claude QA 输出

### 深度文专用补充规则

- 官方来源优先于媒体、公司页、社区报告
- 先处理法规、生效日、适用对象，再处理表达
- 对 gray area 必须明确写成 gray，不得写成 green
- 冲突信息不强行抹平，要明确标注条件或不确定性
- 比普通文更重"事实稳健性"，不追求极限压缩
...
- Avoid these unless quoting a source or unless the exact term is strictly necessary for accuracy
- If one appears in the draft, replace it with plainer wording by default
...
- Do not compress away a real condition merely to make the writing feel smoother
...
- Avoid obvious "SEO article" phrasing
- Avoid obvious "AI-generated summary" phrasing
...
## Prompt 4：Draft + SEO/GEO Enhanced
...
- Do not pad sections just to make them longer
- Prioritize decision-usefulness over completeness
- For normal articles, avoid including too many high-volatility claims, named entities, or rapidly changing market details unless they are essential to the main decision
