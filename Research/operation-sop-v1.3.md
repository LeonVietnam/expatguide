...
1. SOP 版本号同步升级到 **v2.53**(含 Scope Definition Check、Decision Data Floor、完整 Frontmatter Block 输出)
...
```
按 SOP v2.53 Prompt 1 的格式,帮我生成 10 个普通车道选题提案。
...
```
按 SOP v2.53 Prompt 1 的格式,帮我生成 1 个选题提案(不是 10 个选项)。
...
```
Topic Sheet 通过。请按 SOP v2.53 Prompt 2 执行,输出完整的 RESEARCH PACK。
```
...
```
Research Pack 确认。请按 SOP v2.53 Prompt 3 执行 adversarial validation,输出完整 VALIDATION PACK。
```
...
```
Validation Pack 确认。现在按 SOP v2.53 Prompt 4 执行:

1. 先输出完整的 DRAFT PACKAGE(包括 Self-Audit 带证据部分)
2. Self-Audit 完成后,立刻扮演 Prompt 5 的 issue hunter 角色,对照 SOP v2.53 规则找出自己 draft 中的所有问题:
   - 硬禁用词、语境敏感词使用是否合理
...
```
以下是 bash check-articles.sh 的兜底输出,请按 SOP v2.53 Prompt 5 最小改动规则修复列出的 Errors,其他地方保持不变。
