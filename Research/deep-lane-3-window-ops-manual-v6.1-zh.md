# ExpatGuide 深度车道 3 窗口操作手册 v6.1（GPT-only 生产版）

## 文档定位

这份文档**给操作者看**，用于深度文章从选题到发布的逐步操作。

它不是项目知识库文件。项目知识库应放对应 SOP / mini-spec 文件，见《ChatGPT 项目配置与指令方案 v6.1》。

## v6.1 更新重点

1. Deep Draft 项目知识更新为 `expatguide_deep_sop_gpt_v1.4.md` + `deep-draft-window-spec-v5.4.md`。
2. Step 7 改为生成 `risk-qa-handoff-[article-slug].md`，聊天正文不再直接展开超长 Risk QA handoff block。
3. Step 8 改为在 Deep Risk QA 项目上传该 handoff md 文件后执行 Risk QA。
4. Step 9 改为生成 `patch-verify-handoff-[article-slug].md`，聊天正文不再直接展开超长 Patch Verify handoff block。
5. Step 10 改为在 Deep Patch Verify 项目上传该 handoff md 文件后执行 Patch Verification。
6. 两类 handoff md 文件必须完整保留原始 block 内容，不允许 summary / omission / shortening / placeholder。

## 上一轮 safeguards 更新重点

1. Deep Draft 项目知识当前使用 `expatguide_deep_sop_gpt_v1.4.md` + `deep-draft-window-spec-v5.4.md`。
...
- Deep Draft：`expatguide_deep_sop_gpt_v1.4.md`、`deep-draft-window-spec-v5.4.md`、最新 `internal-articles-index.md`
...
7. 窗口 A：Deep Claim Sheet + Risk QA Handoff md 文件
8. 窗口 B：Risk QA + Source Spot Check
9. 窗口 A：Patch / Final Package + Patch Verify Handoff md 文件
...
# Step 7：窗口 A 生成 Claim Sheet + Risk QA Handoff md 文件
...
1. DEEP CLAIM SHEET
2. 生成一个完整 markdown 文件：risk-qa-handoff-[article-slug].md
...
risk-qa-handoff-[article-slug].md 文件内容必须完整包含 RISK QA HANDOFF BLOCK，且必须包含：
- TOPIC SHEET
- DEEP RESEARCH PACK
...
聊天正文不要直接展开 handoff block，只输出：
- 文件名
- 文件内 6 个必需 section 的 checklist
- 下一步给 Deep Risk QA 项目的 prompt
...
- handoff md 文件是否 6 段齐全。
- 文件内是否包含完整 `=== RISK QA HANDOFF BLOCK START/END ===` 边界。
- 不要把单篇 handoff md 上传进 Project knowledge；它只用于对应文章的一次 QA 输入。
...
```text
请读取上传的 risk-qa-handoff-[article-slug].md。
该文件包含完整 RISK QA HANDOFF BLOCK。
...
2. 生成一个完整 markdown 文件：patch-verify-handoff-[article-slug].md

patch-verify-handoff-[article-slug].md 文件内容必须完整包含 PATCH VERIFY HANDOFF BLOCK，并使用：
=== PATCH VERIFY HANDOFF BLOCK START ===
...
=== PATCH VERIFY HANDOFF BLOCK END ===

聊天正文不要直接展开 handoff block，只输出文件名、section checklist、下一步给 Deep Patch Verify 项目的 prompt。
````
...
- 同时生成一个完整 markdown 文件：patch-verify-handoff-[article-slug].md。
- 文件内容必须完整包含 PATCH VERIFY HANDOFF BLOCK；聊天正文不要直接展开 handoff block。
````
...
```text
请读取上传的 patch-verify-handoff-[article-slug].md。
该文件包含完整 PATCH VERIFY HANDOFF BLOCK。
