# ExpatGuide GPT-Only 深度文章生产 SOP v1.4
...
本版将 deep lane 的两个长交接包改为 markdown 文件交付：

1. Draft 后生成 `risk-qa-handoff-[article-slug].md`，文件内完整包含 `RISK QA HANDOFF BLOCK`。
2. Patch / Final 后生成 `patch-verify-handoff-[article-slug].md`，文件内完整包含 `PATCH VERIFY HANDOFF BLOCK`。
3. 聊天正文不直接展开超长 handoff block，只输出文件名、section checklist 和下一步 prompt。
4. handoff 文件内容仍必须完整，不能 summary / omission / shortening / placeholder。

## Frontmatter / affiliate / QA safeguard 更新重点

本轮 safeguards 修补首篇深度文 `vietnam-health-insurance-expats-2026` 实战暴露的生产缺口：

1. Frontmatter 字段白名单硬规则。
2. Affiliate 链接 URL 模板。
3. Risk QA 强制检查 Decision Data Floor。
4. Risk QA 强制检查 Eligibility Matrix 真实性。
5. Patch Verify 强制检查 frontmatter 和 affiliate 链接。

## Source Spot Check evidence 更新重点

本轮 evidence 规则继续修补正式测试前暴露的风险点：
...
- Affiliate 链接必须使用标准 URL，不允许使用普通官网链接：
...
- affiliate_url_template_compliance
- frontmatter_field_whitelist
...
## RISK QA handoff md 文件

Deep Draft 输出 Draft 和 Claim Sheet 后，必须额外生成完整交接文件。
...
聊天正文不要直接展开完整 block，只输出文件名、6 个 section checklist 和下一步 prompt。用户上传该 md 文件到 Deep Risk QA 项目。

如果当前环境无法生成文件，才退回分 PART 输出完整 RISK QA HANDOFF BLOCK；不得 summary / omission / shortening / placeholder。

---

## Deep Risk QA 要求

Deep Risk QA 项目以 `deep-risk-qa-mini-spec-v4.2.md` 为唯一细则来源。
...
- Source Spot Check 必须遵守 `deep-risk-qa-mini-spec-v4.2.md` 的 evidence quality rule。
...
- Risk QA 必须主动检查 Decision Data Floor 是否被满足：产品 / 服务 / 选项对比类文章必须包含价格区间 / 时效区间 / 落地信息 / 波动 caveat 四类数据。缺失任一类自动创建 High issue。
- Risk QA 必须主动检查 Eligibility Matrix 是否真的实现为硬资格矩阵：应包含 visa / 年龄 / 雇佣状态等硬条件 + yes / no / conditional 判定语言。退化为软推荐表自动创建 High issue。
...
## PATCH VERIFY handoff md 文件

Deep Draft 输出 Final Package 后，必须额外生成完整交接文件。
...
聊天正文不要直接展开完整 block，只输出文件名、7 个 section checklist 和下一步 prompt。用户上传该 md 文件到 Deep Patch Verify 项目。

如果当前环境无法生成文件，才退回分 PART 输出完整 PATCH VERIFY HANDOFF BLOCK；不得 summary / omission / shortening / placeholder。

---

## Patch Verify 要求

Patch Verify 项目以 `deep-patch-verify-mini-spec-v4.1.md` 为唯一细则来源。

Patch Verify 必须严格检查 frontmatter 字段白名单和 affiliate 链接 URL 格式，任一不合规自动创建 High issue。
...
10. 预览正常。
11. 内链合法。
12. affiliate 合规，且 markdown 链接使用标准 affiliate URL。
13. markdown / frontmatter 合法，且 frontmatter 只含白名单字段。
